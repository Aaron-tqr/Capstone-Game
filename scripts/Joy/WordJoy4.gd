...
# Correct words for joy level 4
var correct_words = ["delighted", "glad", "merry", "joyful"]
...
func _ready():
	...
	print("WordJoy4 drop_target chosen:", drop_target)
	...
	if drop_target and (not drop_target.has_method("receive_drop") or not drop_target.has_signal("word_dropped")):
		print("WordJoy4: drop_target missing API; forcing DropTargetWord script")
	...
	# Setup drop target
	if not drop_target:
		push_error("WordJoy4: drop_target not found")
		return
	...
	if not drop_target.has_signal("word_dropped"):
		push_error("WordJoy4: drop_target does not have word_dropped signal (found: %s)" % drop_target.get_script())
		return
	drop_target.word_dropped.connect(_on_word_dropped)
	pause_button.pressed.connect(_on_pause_pressed)
	level_label.text = "LEVEL %d" % GameState.current_level
	print("📝 WordJoy4 READY! Draggable labels: ", draggable_labels.size(), " | Matching words: ", matching_words.size())
