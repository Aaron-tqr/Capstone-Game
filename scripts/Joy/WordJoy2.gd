...
# Correct words for joy level 2
var correct_words = ["glad", "joyful"]

func _ready():
	print("WordJoy2.gd _ready running with node: ", name, " script: ", get_script().resource_path)
	...
	print("WordJoy2 drop_target chosen:", drop_target)
	...
	if drop_target and (not drop_target.has_method("receive_drop") or not drop_target.has_signal("word_dropped")):
		print("WordJoy2: drop_target missing API; forcing DropTargetWord script")
	...
	# Setup drop target
	if not drop_target:
		push_error("WordJoy2: drop_target not found")
		return
	...
	if not drop_target.has_signal("word_dropped"):
		push_error("WordJoy2: drop_target does not have word_dropped signal (found: %s)" % drop_target.get_script())
		return
	drop_target.word_dropped.connect(_on_word_dropped)
	pause_button.pressed.connect(_on_pause_pressed)
	level_label.text = "LEVEL %d" % GameState.current_level
	print("📝 WordJoy2 READY! Draggable labels: ", draggable_labels.size(), " | Matching words: ", matching_words.size())
