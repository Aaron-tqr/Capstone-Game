...
@onready var fear_char = $FearChar
...
# Correct words for fear (level 3)
var correct_words = ["terrified", "fear", "nervous"]
...
func _ready():
	...
	var bg_node: TextureRect = get_node_or_null("Background") as TextureRect
		if bg_node != null and ResourceLoader.exists(revised_path):
			...
			if tex:
				bg_node.texture = tex
		# Animate in fear character
	if fear_char:
		fear_char.modulate.a = 0
		create_tween().set_trans(Tween.TRANS_QUAD).set_ease(Tween.EASE_OUT).tween_property(fear_char, "modulate:a", 1.0, 0.6)
		# Float bob animation
		_create_float_bob(fear_char, 2.5, 10.0)
	
	# Resolve drop target node
	drop_target = _find_drop_target()
	print("WordFear3 drop_target chosen:", drop_target)
	...
	if drop_target and (not drop_target.has_method("receive_drop") or not drop_target.has_signal("word_dropped")):
		print("WordFear3: drop_target missing API; forcing DropTargetWord script")
	...
	# Setup drop target
	if not drop_target:
		push_error("WordFear3: drop_target not found")
		return
	...
	if not drop_target.has_signal("word_dropped"):
		push_error("WordFear3: drop_target does not have word_dropped signal (found: %s)" % drop_target.get_script())
		return
	...
	print("📝 WordFear3 READY! Draggable labels: ", draggable_labels.size(), " | Matching words: ", matching_words.size())

func _find_drop_target():
	print("WordFear3 _find_drop_target called")
	var candidate = null
