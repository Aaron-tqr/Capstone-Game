...
# Correct words for fear (level 1)
var correct_words = ["fear"]
...
func _ready():
	...
	print("WordFear1 drop_target chosen:", drop_target)
	...
	if drop_target and (not drop_target.has_method("receive_drop") or not drop_target.has_signal("word_dropped")):
		print("WordFear1: drop_target missing API; forcing DropTargetWord script")
	...
	# Setup drop target
	if not drop_target:
		push_error("WordFear1: drop_target not found")
		return
	...
	if not drop_target.has_signal("word_dropped"):
		push_error("WordFear1: drop_target does not have word_dropped signal (found: %s)" % drop_target.get_script())
		return
	...
	print("📝 WordFear1 READY! Draggable labels: ", draggable_labels.size(), " | Matching words: ", matching_words.size())

func _find_drop_target():
	print("WordFear1 _find_drop_target called")
	var candidate = null
