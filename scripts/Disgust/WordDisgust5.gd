...
# Correct words for disgust (level 5)
var correct_words = ["yucky", "nasty", "disgust", "eww", "icky"]
...
func _ready():
	...
	print("WordDisgust5 drop_target chosen:", drop_target)
	...
	if drop_target and (not drop_target.has_method("receive_drop") or not drop_target.has_signal("word_dropped")):
		print("WordDisgust5: drop_target missing API; forcing DropTargetWord script")
	...
	# Setup drop target
	if not drop_target:
		push_error("WordDisgust5: drop_target not found")
		return
	...
	if not drop_target.has_signal("word_dropped"):
		push_error("WordDisgust5: drop_target does not have word_dropped signal (found: %s)" % drop_target.get_script())
		return
	...
	print("📝 WordDisgust5 READY! Draggable labels: ", draggable_labels.size(), " | Matching words: ", matching_words.size())

func _find_drop_target():
	print("WordDisgust5 _find_drop_target called")
	var candidate = null
