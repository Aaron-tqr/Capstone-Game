...
@onready var disgust_char = $DisgustChar
...
# Correct words for disgust (level 3)
var correct_words = ["eww", "icky", "nasty"]
...
func _ready():
	...
	var bg_node: TextureRect = get_node_or_null("Background") as TextureRect
		if bg_node != null and ResourceLoader.exists(revised_path):
			...
			if tex:
				bg_node.texture = tex
		# Animate in disgust character
	if disgust_char:
		disgust_char.modulate.a = 0
		create_tween().set_trans(Tween.TRANS_QUAD).set_ease(Tween.EASE_OUT).tween_property(disgust_char, "modulate:a", 1.0, 0.6)
		# Float bob animation
		_create_float_bob(disgust_char, 2.5, 10.0)
	
	# Resolve drop target node
	drop_target = _find_drop_target()
	print("WordDisgust3 drop_target chosen:", drop_target)
	...
	if drop_target and (not drop_target.has_method("receive_drop") or not drop_target.has_signal("word_dropped")):
		print("WordDisgust3: drop_target missing API; forcing DropTargetWord script")
	...
	_collect_word_labels(self, "LEVEL 3")
	
	# Setup drop target
	if not drop_target:
		push_error("WordDisgust3: drop_target not found")
		return
	...
	if not drop_target.has_signal("word_dropped"):
		push_error("WordDisgust3: drop_target does not have word_dropped signal (found: %s)" % drop_target.get_script())
		return
	drop_target.word_dropped.connect(_on_word_dropped)
	pause_button.pressed.connect(_on_pause_pressed)
	WordHintHelper.attach(self)
	level_label.text = "LEVEL %d" % GameState.current_level
	print("📝 WordDisgust3 READY! Draggable labels: ", draggable_labels.size(), " | Matching words: ", matching_words.size())

func _find_drop_target():
	print("WordDisgust3 _find_drop_target called")
	var candidate = null
