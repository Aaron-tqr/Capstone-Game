...
var correct_words = ["sorrowful", "unhappy", "despair"]

func _ready():
	print("WordSadness3.gd _ready running with node: ", name, " script: ", get_script().resource_path)
	...
	print("WordSadness3 drop_target chosen:", drop_target)
	...
	if drop_target and (not drop_target.has_method("receive_drop") or not drop_target.has_signal("word_dropped")):
		print("WordSadness3: drop_target missing API; forcing DropTargetWord script")
	...
	# Setup drop target
	if not drop_target:
		push_error("WordSadness3: drop_target not found")
		return
	...
	if not drop_target.has_signal("word_dropped"):
		push_error("WordSadness3: drop_target does not have word_dropped signal (found: %s)" % drop_target.get_script())
		return
	...
	print("📝 WordSadness3 READY! Draggable labels: ", draggable_labels.size(), " | Matching words: ", matching_words.size())
