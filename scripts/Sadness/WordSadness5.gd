...
var correct_words = ["despair", "grief", "depressed", "heartbroken", "melancholy"]

func _ready():
	print("WordSadness5.gd _ready running with node: ", name, " script: ", get_script().resource_path)
	...
	print("WordSadness5 drop_target chosen:", drop_target)
	...
	if drop_target and (not drop_target.has_method("receive_drop") or not drop_target.has_signal("word_dropped")):
		print("WordSadness5: drop_target missing API; forcing DropTargetWord script")
	...
	# Setup drop target
	if not drop_target:
		push_error("WordSadness5: drop_target not found")
		return
	...
	if not drop_target.has_signal("word_dropped"):
		push_error("WordSadness5: drop_target does not have word_dropped signal (found: %s)" % drop_target.get_script())
		return
	...
	print("📝 WordSadness5 READY! Draggable labels: ", draggable_labels.size(), " | Matching words: ", matching_words.size())
